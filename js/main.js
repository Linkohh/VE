...
const VibeMe = {
  ...
  updateQuote: function () {
    ...
    if (quoteText) {
      ...
      setTimeout(() => {
        // Kinetic Typography: Split into words
        const words = quote.text.split(' ');
        const html = words.map((word, i) =>
          `<span class="word-anim" style="animation-delay: ${i * 0.05}s">${word}</span>`
        ).join(' ');
        ...
        }, 800 + (words.length * 50)); // Adjust duration based on word count
      }, 400);
    }
  ...
  applyTheme: function (theme) {
    ...
    // Update Aura Glow colors
    root.style.setProperty('--glow-outer-color', theme.color1);
    root.style.setProperty('--glow-inner-color', theme.color2);
...
function applyPalette({ color1, color2, color3, accent }) {
  ...
  // Update Aura Glow colors
  root.style.setProperty('--glow-outer-color', color1);
  root.style.setProperty('--glow-inner-color', color2);
}
...
(function () {
  ...
  function renderList() {
    ...
    items.forEach((q, idx) => {
      ...
      row.innerHTML = `
        ...
        <div class="fav-actions">
          <button class="fav-view" title="View Quote"><i class="fas fa-eye"></i></button>
    ...
    listEl.onclick = async (ev) => {
      ...
      if (btn.classList.contains('fav-view')) {
        // VIEW: load into main quote box
        const quoteText = document.getElementById('quote-text');
        const quoteAuthor = document.getElementById('quote-author');

        if (quoteText && quoteAuthor) {
          // Update text content
          quoteText.textContent = target.text;
          quoteAuthor.textContent = target.author ? `— ${target.author}` : '';
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Vibrancy Slider - Update glow intensity
    const vibrancySlider = $('#vibrancy-slider');
    const vibrancyValue = $('#vibrancy-value');
    if (vibrancySlider && vibrancyValue) {
      vibrancySlider.addEventListener('input', () => {
        const value = vibrancySlider.value;
        vibrancyValue.textContent = `${value}%`;
        // Update glow intensity (0-100% maps to 0-1)
        const intensity = parseFloat(value) / 100;
        document.documentElement.style.setProperty('--glow-intensity-mult', intensity);
      });
    }

    // Accessibility Mode Toggle
    const accessibilityToggle = $('#accessibility-mode');
    ...
    // ===== RADIAL SHARE MENU LOGIC =====
    const shareMenu = $('#shareMenu');
    const shareToggleBtn = $('#shareToggleBtn');
    const shareToast = $('#share-toast');
    const shareToastMsg = $('#share-toast-msg');
    let shareInactivityTimer;
    let shareToastTimeout;

    if (shareMenu && shareToggleBtn) {
      const openMenu = () => {
      ...
      const closeMenu = () => {
        ...
        clearTimeout(shareInactivityTimer);
      };
      ...
      const resetShareTimer = () => {
        clearTimeout(shareInactivityTimer);
        if (shareMenu.classList.contains('active')) {
          shareInactivityTimer = setTimeout(closeMenu, 6000); // 6s auto-close
        }
      };

      const showToast = (msg) => {
        if (!shareToast || !shareToastMsg) return;
        shareToastMsg.textContent = msg;
      ...
      shareToggleBtn.addEventListener('click', (e) => {
        ...
        shareToggleBtn.classList.toggle('active');
      });

      // Close menu when clicking outside
      document.addEventListener('click', (e) => {
        if (!shareMenu.contains(e.target) && shareMenu.classList.contains('active')) {
          shareMenu.classList.remove('active');
      ...
      const shareActions = {
        'twitter': () => {
          const text = encodeURIComponent(`"${VibeMe.getCurrentQuote().text}" — ${VibeMe.getCurrentQuote().author}`);
        ...
        'email': () => {
          const subject = encodeURIComponent("Thought you'd like this vibe");
          const body = encodeURIComponent(`"${VibeMe.getCurrentQuote().text}" — ${VibeMe.getCurrentQuote().author}\n\nSent from VibeMe`);
        ...
        'copy': async () => {
          const text = `"${VibeMe.getCurrentQuote().text}" — ${VibeMe.getCurrentQuote().author}`;
          try {
            await navigator.clipboard.writeText(text);
            showShareToast('Quote Copied!');
          } catch (err) {
      ...
      document.querySelectorAll('.action-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
          ...
          // Identify action based on class
          if (btn.classList.contains('btn-twitter')) shareActions.twitter();
          else if (btn.classList.contains('btn-facebook')) shareActions.facebook();
          else if (btn.classList.contains('btn-email')) shareActions.email();
          else if (btn.classList.contains('btn-whatsapp')) shareActions.whatsapp();
          else if (btn.classList.contains('btn-copy')) shareActions.copy();
