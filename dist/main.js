...
// js/main.ts - VibeMe Enhanced TypeScript (No Modules)
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (isFile) {
                ...
                if (window.__QUOTES_JS_PROMISE) {
                    const loaded = yield window.__QUOTES_JS_PROMISE;
                ...
                try {
                    const res = yield fetch('data/quotes.json', { cache: 'no-cache' });
                    if (res.ok) {
...
(function () {
    try {
        if (window.__VM_AUDIO_GUARD_PATCHED)
            return;
        ...
        if (NativeAC) {
            const PatchedAC = function (...args) {
                ...
                (window.__allAudioContexts || (window.__allAudioContexts = [])).push(ctx);
                return ctx;
            };
        ...
        const isBeepEnabled = () => {
            try {
                return !(window.VibeMe && VibeMe.state && VibeMe.state.beepEnabled === false);
            }
            catch (_) {
                return true;
            }
...
const VibeMe = {
    ...
    normalizeQuotes: function (raw) {
        ...
        function sanitize(arr, defaultCategory) {
            return (arr || [])
                ...
                var _a, _b, _c, _d;
                return ({
                    text: String((_b = (_a = q.text) !== null && _a !== void 0 ? _a : q.quote) !== null && _b !== void 0 ? _b : '').trim(),
                    author: String((_c = q.author) !== null && _c !== void 0 ? _c : 'Unknown').trim(),
                    category: String((_d = defaultCategory !== null && defaultCategory !== void 0 ? defaultCategory : q.category) !== null && _d !== void 0 ? _d : 'default').trim()
                });
            })
        ...
        if (raw && raw.categories && typeof raw.categories === 'object') {
            ...
            for (const [cat, list] of Object.entries(raw.categories)) {
                if (Array.isArray(list))
                    combined.push(...sanitize(list, cat));
            }
            return combined;
        }
        // Unknown shape → empty
    ...
    onQuotesReady: function (source) {
        ...
        if (this.bus && this.bus.emit) {
            try {
                this.bus.emit('quotes:ready', { source, count: this.quotes.length });
            }
            catch (_) { }
        }
    },
    loadQuotes: function () {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const quotesData = yield window.VIBE_QUOTES_PROMISE;
    ...
    updateQuote: function () {
        ...
        setTimeout(() => {
            var _a, _b;
            const __payload = {
                quote: (((_a = VibeMe.kit.$('#quote-text')) === null || _a === void 0 ? void 0 : _a.textContent) || '').trim(),
                author: (((_b = VibeMe.kit.$('#quote-author')) === null || _b === void 0 ? void 0 : _b.textContent) || '').trim()
            };
            if (typeof VibeMe.bus !== 'undefined') {
    ...
    toggleFavorite: function () {
        ...
        const existingIndex = this.state.favorites.findIndex(fav => fav.text === quote.text && fav.author === quote.author);
        if (existingIndex >= 0) {
    ...
    expandShareButtons: function () {
        ...
        if (!shareHubBtn || !buttonContainer)
            return;
    ...
    resetShareButton: function () {
        const buttonContainer = document.getElementById('shareFanContainer');
        const shareHubBtn = document.getElementById('shareHubBtn');
        if (!buttonContainer || !shareHubBtn)
            return;
    ...
    playSound: function (type) {
        if (!this.audioContext || !this.state.effectsEnabled)
            return;
        const frequencies = {
            click: 800,
            generate: 600,
    ...
    getActiveBlendMode: function () {
        const body = document.body;
        if (body.classList.contains('matrix-mode-high-contrast'))
            return 'high-contrast';
        if (body.classList.contains('matrix-mode-light-bg'))
            return 'light-bg (multiply)';
        if (body.classList.contains('matrix-mode-dark-bg'))
            return 'dark-bg (screen)';
        return 'none';
    },
    ...
    generateVibriantPalette: function (baseHue, vibrancy = 0.8, warmth = 0.5) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            colors.push(this.hslToHex(hue, Math.max(20, Math.min(95, sat)), Math.max(25, Math.min(85, light))));
        }
        return colors;
    },
    // Intelligent theme generator that chooses the best harmony type
    generateIntelligentTheme: function (options = {}) {
        const { harmonyType = 'auto', vibrancy = 0.7, warmth = 0.5, accessibility = true, baseHue = null } = options;
        ...
        if (harmonyType === 'auto') {
            ...
            switch (chosenHarmony) {
                case 'analogous':
                    colors = this.generateAnalogousColors(hue, saturation, lightness);
                    break;
                case 'triadic':
                    colors = this.generateTriadicColors(hue, saturation, lightness);
                    break;
                case 'complementary':
                    colors = this.generateComplementaryColors(hue, saturation, lightness);
                    break;
                case 'vibrant':
                    colors = this.generateVibriantPalette(hue, vibrancy, warmth);
                    break;
            ...
            switch (harmonyType) {
                ...
                case 'split-complementary':
                    colors = this.generateSplitComplementaryColors(hue, saturation, lightness);
                    break;
                case 'tetradic':
                    colors = this.generateTetradicColors(hue, saturation, lightness);
                    break;
                case 'monochromatic':
                    colors = this.generateMonochromaticColors(hue, saturation, lightness);
                    break;
                case 'vibrant':
                    colors = this.generateVibriantPalette(hue, vibrancy, warmth);
                    break;
    ...
    setupMatrixEffect: function () {
        ...
        if (!this.matrixBg || !this.state.effectsEnabled)
            return;
    ...
    applyMatrixThemeColors: function (column) {
        if (!this.matrixConfig.colors || this.matrixConfig.colors.length === 0)
            return;
    ...
    setupEventListeners: function () {
        ...
        if (renderModeSelector) {
            ...
            this.toggleCanvasPerformanceSettings(renderModeSelector.value === 'canvas' || renderModeSelector.value === 'hybrid');
        }
        // Settings tabs
    ...
    rateQuote: function (direction) {
        ...
        if (direction === 'up')
            this.state.quoteRatings[quoteId].up++;
        else
            this.state.quoteRatings[quoteId].down++;
        this.saveRatings();
        this.updateRatingDisplay();
        this.playSound('click');
    },
    ...
    updateFavoriteButton: function (quote) {
        ...
        if (!icon)
            return;
        const isFavorite = this.state.favorites.findIndex(fav => fav.text === quote.text && fav.author === quote.author) >= 0;
        if (isFavorite) {
            icon.classList.remove('far');
            icon.classList.add('fas');
            favoriteBtn.setAttribute('aria-pressed', 'true');
    ...
    removeSingleMatrixColumn: function (index) {
        ...
        if (!col)
            return;
        try {
            if (col.el && col.el.parentNode)
                col.el.parentNode.removeChild(col.el);
        }
        catch (_) { }
        this.matrixState.activeColumns.splice(index, 1);
    },
    ...
    drawCanvasMatrix: function () {
        ...
        // trailing fade layer
        ctx.fillStyle = `rgba(0,0,0,${1 - ((_a = cfg.globalOpacity) !== null && _a !== void 0 ? _a : 0.9)})`;
        ...
        for (let i = 0; i < this.matrixState.canvasDrops.length; i++) {
            const text = this.matrixConfig.characters[Math.floor(Math.random() * this.matrixConfig.characters.length)];
    ...
    refreshCanvasColors: function () {
        if (!this.matrixState.canvas || !this.matrixState.canvasDrops.length)
            return;
...
VibeMe.kit = VibeMe.kit || {
    ...
    bind(el, type, handler, options) { if (el)
        el.addEventListener(type, handler, options); },
    delegate(root, event, selector, handler) {
        if (!root)
            return;
        this.bind(root, event, (e) => { var _a; const m = (_a = e.target) === null || _a === void 0 ? void 0 : _a.closest(selector); if (m && root.contains(m))
            handler(e, m); });
    }
};
// Add no-op stubs for matrix animation functions to prevent TypeError
VibeMe.stopMatrixAnimation || (VibeMe.stopMatrixAnimation = () => { });
VibeMe.startMatrixAnimation || (VibeMe.startMatrixAnimation = () => { });
VibeMe.bus = VibeMe.bus || (() => {
    const bus = new EventTarget();
    return {
        emit(eventName, detail) { try {
            bus.dispatchEvent(new CustomEvent(eventName, { detail }));
        }
        catch (e) {
            console.warn('[VibeBus]', e);
        } },
        on(eventName, handler) { bus.addEventListener(eventName, handler); }
    };
})();
VibeMe.features = Object.assign(Object.assign({}, (VibeMe.features || {})), { tts: true, effects: true, matrix: true });
// Add voiceURI to persisted TTS settings
VibeMe.settings = VibeMe.settings || {};
VibeMe.settings.tts = Object.assign(Object.assign({}, (VibeMe.settings.tts || {})), { enabled: ((_b = (_a = VibeMe.settings.tts) === null || _a === void 0 ? void 0 : _a.enabled) !== null && _b !== void 0 ? _b : true), rate: ((_d = (_c = VibeMe.settings.tts) === null || _c === void 0 ? void 0 : _c.rate) !== null && _d !== void 0 ? _d : 1.0), voiceURI: ((_f = (_e = VibeMe.settings.tts) === null || _e === void 0 ? void 0 : _e.voiceURI) !== null && _f !== void 0 ? _f : null) });
VibeMe.tts = VibeMe.tts || {
    ...
    speak(text, opts = {}) {
        var _a, _b, _c, _d;
        try {
            if (!VibeMe.features.tts || !('speechSynthesis' in window) || !text)
                return;
            const u = new SpeechSynthesisUtterance(text);
            const rate = (_a = opts.rate) !== null && _a !== void 0 ? _a : VibeMe.settings.tts.rate;
            const pitch = (_b = opts.pitch) !== null && _b !== void 0 ? _b : 1;
            const volume = (_c = opts.volume) !== null && _c !== void 0 ? _c : 1;
            const lang = (_d = opts.lang) !== null && _d !== void 0 ? _d : (document.documentElement.lang || 'en-US');
            u.rate = rate;
            u.pitch = pitch;
            u.volume = volume;
            u.lang = lang;
...
const settingsManager = {
    load() {
        ...
        try {
            ...
            if (saved === null || saved === void 0 ? void 0 : saved.tts)
                VibeMe.settings.tts = Object.assign(Object.assign({}, VibeMe.settings.tts), saved.tts);
        }
        catch (e) {
            console.warn('[settings] parse', e);
        }
    ...
    applyToUI() {
        ...
        if (rateEl)
            rateEl.value = rate;
        if (rateLbl)
            rateLbl.textContent = `${parseFloat(rate).toFixed(1)}x`;
    }
};
...
VibeMe.tts.getVoicesSafe = function () {
    var _a, _b;
    try {
        return ((_b = (_a = window.speechSynthesis) === null || _a === void 0 ? void 0 : _a.getVoices) === null || _b === void 0 ? void 0 : _b.call(_a)) || [];
    }
    catch (_c) {
        return [];
    }
...
VibeMe.kit.bind(VibeMe.kit.$('#tts-preview-btn'), 'click', () => {
    ...
    VibeMe.tts.init();
    const sel = VibeMe.kit.$('#tts-voice');
    const selectedUri = (sel === null || sel === void 0 ? void 0 : sel.value) || null;
...
function applyPalette({ color1, color2, color3, accent }) {
    ...
    if (accent)
        root.style.setProperty('--social-icon-bg', accent);
    ...
    if (glow)
        glow.style.setProperty('--glow-color', color1);
}
function pickPaletteFromPreset(name) {
    var _a, _b;
    const bank = (_b = (_a = VibeMe === null || VibeMe === void 0 ? void 0 : VibeMe.themes) === null || _a === void 0 ? void 0 : _a.colorPalettes) === null || _b === void 0 ? void 0 : _b[name];
    if (!bank || !bank.length)
        return null;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (presetSelect) {
        ...
        function activatePreset(name) {
            ...
            if (name === 'auto') {
                const idx = ((_a = VibeMe === null || VibeMe === void 0 ? void 0 : VibeMe.state) === null || _a === void 0 ? void 0 : _a.currentQuoteIndex) || 0;
                const cat = ((_c = (_b = VibeMe === null || VibeMe === void 0 ? void 0 : VibeMe.quotes) === null || _b === void 0 ? void 0 : _b[idx]) === null || _c === void 0 ? void 0 : _c.category) || 'default';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (h1 && quoteInner && quoteInner.contains(h1))
        bar.appendChild(h1);
    if (!h1) {
    ...
    function autoSpace() {
        ...
        if (h < 700)
            bar.classList.add('title-tight');
        else if (h > 980)
            bar.classList.add('title-roomy');
        else
            bar.classList.add('title-comfy'); // comfy default
    }
    autoSpace();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!underline) {
        ...
        if (h1 && h1.nextSibling)
            h1.parentNode.insertBefore(underline, h1.nextSibling);
        else
...
VibeMe.applyMatrixPreset = function (name) {
    ...
    if (VibeMe.matrixConfig) {
        ...
        if (typeof VibeMe.reinitMatrix === 'function')
            VibeMe.reinitMatrix();
        if (typeof VibeMe.updateMatrixConfig === 'function')
            VibeMe.updateMatrixConfig(VibeMe.matrixConfig);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function activate(name) {
        if (name === 'auto') {
            ...
            const cat = (currentQuote === null || currentQuote === void 0 ? void 0 : currentQuote.category) || 'default';
    ...
    document.addEventListener('vibeme:quote:changed', () => {
        if ((localStorage.getItem('vibeme-matrix-preset') || 'auto') === 'auto')
            activate('auto');
    });
...
(function () {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const findVisibilityGroup = () => {
            ...
            if (g)
                return g.closest('.space-y-2, .mt-3, .mb-3, div');
            // by label text
            const lbl = Array.from(panel.querySelectorAll('label, p, span')).find(el => /matrix\s*visibility/i.test(el.textContent || ''));
      ...
      `;
            ...
            if (opt)
                opt.textContent = '🌧️ Rain Matrix (Neon)';
        }
        // 3e) Keep persistence and activation (if your earlier code exists, this is a no-op)
        const saved = localStorage.getItem('vibeme-matrix-preset') || 'auto';
        if (preset)
            preset.value = saved;
        if (typeof (VibeMe === null || VibeMe === void 0 ? void 0 : VibeMe.applyMatrixPreset) === 'function') {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!VibeMe._getRandomQuote && typeof VibeMe.getRandomQuote === 'function') {
        ...
        VibeMe.getRandomQuote = function () {
            ...
            const filter = (((_a = VibeMe.state) === null || _a === void 0 ? void 0 : _a.categoryFilter) || 'all');
            if (filter === 'all')
                return base();
...
(function () {
    ...
    if (typeof VibeMe !== 'undefined' && VibeMe.showFeedback && !VibeMe._showFeedback) {
        ...
        VibeMe.showFeedback = function (message, type = 'info') {
            // Call original
            if (VibeMe._showFeedback)
                VibeMe._showFeedback(message, type);
            // Also show toast
...
(function () {
    if (window.FcClock)
        return; // Guard against re-declaration
    'use strict';
    function FcPiece(label, value) {
        ...
        this.el = el;
        var top = el.querySelector('.fc-top'), bottom = el.querySelector('.fc-bottom'), back = el.querySelector('.fc-back'), backBottom = el.querySelector('.fc-back .fc-bottom');
    ...
    function FcClock(updateFn) {
        ...
        Object.keys(state).forEach(function (k) {
            if (k === 'Total' || k === 'Meridiem')
                return; // no flip tile for AM/PM
            map[k] = new FcPiece(k, state[k]);
            wrap.appendChild(map[k].el);
        });
        if (map.Hours)
            map.Hours.el.setAttribute('data-sep', ':');
        if (map.Minutes)
            map.Minutes.el.setAttribute('data-sep', ':');
    ...
    function mountFcClock() {
        ...
        if (mount.style && mount.style.width)
...
(function () {
    ...
    function readRaw() {
        var _a, _b;
        const merge = [];
        const push = (val) => { if (Array.isArray(val))
            merge.push(...val); };
        push((_b = (_a = window.VibeMe) === null || _a === void 0 ? void 0 : _a.state) === null || _b === void 0 ? void 0 : _b.favorites);
        try {
            push(JSON.parse(localStorage.getItem('vibeme-favorites') || '[]'));
        }
        catch (_c) { }
        try {
        ...
        try {
            push(JSON.parse(localStorage.getItem('vibemeFavorites') || '[]'));
        }
        catch (_e) { }
        return merge;
    }
    function saveRaw(raw) {
        var _a;
        try {
            localStorage.setItem('vibeme-favorites', JSON.stringify(raw));
        }
        catch (_b) { }
        if ((_a = window.VibeMe) === null || _a === void 0 ? void 0 : _a.state)
            window.VibeMe.state.favorites = raw;
        refreshCount();
    }
    ...
    function normalize(raw) {
        const items = (raw || []).map((it) => {
            var _a, _b, _c, _d, _e;
            if (typeof it === 'string')
                return { text: it, author: null };
            if (it && typeof it === 'object') {
                const text = (_c = (_b = (_a = it.text) !== null && _a !== void 0 ? _a : it.quote) !== null && _b !== void 0 ? _b : it.q) !== null && _c !== void 0 ? _c : '';
                const author = (_e = (_d = it.author) !== null && _d !== void 0 ? _d : it.a) !== null && _e !== void 0 ? _e : null;
                return { text, author };
            }
            return { text: String(it !== null && it !== void 0 ? it : ''), author: null };
        }).filter(x => x.text);
        // de-dupe by text+author
        const seen = new Set();
        return items.filter(x => {
            var _a;
            const k = (x.text + '|' + ((_a = x.author) !== null && _a !== void 0 ? _a : '')).toLowerCase();
            if (seen.has(k))
                return false;
            seen.add(k);
            return true;
    ...
    function favKey(q) {
        ...
        return (((_a = q === null || q === void 0 ? void 0 : q.text) !== null && _a !== void 0 ? _a : '') + '|' + ((_b = q === null || q === void 0 ? void 0 : q.author) !== null && _b !== void 0 ? _b : '')).toLowerCase();
    }
    ...
    function renderList() {
        ...
        // Delegated actions
        listEl.onclick = (ev) => __awaiter(this, void 0, void 0, function* () {
            var _a, _b, _c, _d, _e, _f;
            ...
            if (btn.classList.contains('fav-copy')) {
                ...
                try {
                    yield ((_a = navigator.clipboard) === null || _a === void 0 ? void 0 : _a.writeText(toCopy));
                }
                catch (_g) { }
                return;
            }
            if (btn.classList.contains('fav-remove')) {
                ...
                for (let i = 0; i < raw.length; i++) {
                    ...
                    const norm = (typeof r === 'string')
                        ? { text: r, author: null }
                        : (r && typeof r === 'object')
                            ? { text: ((_d = (_c = (_b = r.text) !== null && _b !== void 0 ? _b : r.quote) !== null && _c !== void 0 ? _c : r.q) !== null && _d !== void 0 ? _d : ''), author: ((_f = (_e = r.author) !== null && _e !== void 0 ? _e : r.a) !== null && _f !== void 0 ? _f : null) }
                            : { text: String(r !== null && r !== void 0 ? r : ''), author: null };
                    if (!norm.text)
                        continue;
                    if (favKey(norm) === keyToRemove) {
                        removeAt = i;
                        break;
                    }
                }
    ...
    (function patchSetItem() {
        ...
        localStorage.setItem = function (k, v) {
            ...
            if (k === 'vibeme-favorites') {
                refreshCount();
                if (((_a = $('favorites-panel')) === null || _a === void 0 ? void 0 : _a.dataset.state) === 'open')
                    renderList();
            }
            return r;
        };
    ...
    function init() {
        ...
        document.addEventListener('keydown', (e) => { if (e.key === 'Escape' && state === 'open')
            apply('closed'); });
        // Cross-tab sync
        window.addEventListener('storage', (ev) => { if (ev.key === 'vibeme-favorites') {
            refreshCount();
            if (state === 'open')
                renderList();
        } });
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (toggle && panel) {
            toggle.addEventListener('click', (e) => {
                ...
                if (!isHidden) {
                    (_a = panel.focus) === null || _a === void 0 ? void 0 : _a.call(panel);
                }
            });
        ...
        if (effectsCb) {
            const apply = (on) => {
                ...
                try {
                    localStorage.setItem('vibeme-effects', JSON.stringify(!!on));
                }
                catch (_a) { }
            };
        ...
        if (darkBtn) {
            darkBtn.addEventListener('click', () => {
                ...
                try {
                    localStorage.setItem('vibeme-dark-mode', JSON.stringify(isDark));
                }
                catch (_a) { }
        ...
        presetSel === null || presetSel === void 0 ? void 0 : presetSel.addEventListener('change', updateThemeColorMeta, { capture: true });
        applyBtn === null || applyBtn === void 0 ? void 0 : applyBtn.addEventListener('click', () => setTimeout(updateThemeColorMeta, 0));
        // Show with '?' key
        document.addEventListener('keydown', (e) => {
            if (e.key === '?')
                openKeyboardHelp();
        ...
        document.addEventListener('keydown', (e) => {
            var _a, _b;
            if (e.key === 'Escape' && !((_a = $('#settings-panel')) === null || _a === void 0 ? void 0 : _a.classList.contains('hidden'))) {
                (_b = $('#settings-toggle')) === null || _b === void 0 ? void 0 : _b.click();
            }
        });
...
(function ensureTTSWiredOnce() {
    if (VibeMe.__ttsWired)
        return;
    VibeMe.__ttsWired = true;
    const speakHandler = (e) => {
        var _a, _b;
        if (!VibeMe.settings.tts.enabled)
            return;
        const q = ((_a = e.detail) === null || _a === void 0 ? void 0 : _a.quote) || '';
        const a = ((_b = e.detail) === null || _b === void 0 ? void 0 : _b.author) ? ` — ${e.detail.author}` : '';
        ...
        try {
            ...
            if (toggle) {
                ...
                toggle.addEventListener('change', function () {
                    ...
                    try {
                        VibeMe.state.beepEnabled = on;
                    }
                    catch (_) { }
                    try {
                        localStorage.setItem('vibeme-beep-enabled', JSON.stringify(on));
                    }
                    catch (_) { }
                    applyBeepAudioState();
                });
    ...
    document.addEventListener('DOMContentLoaded', function () {
        try {
            const guard = (fn) => typeof fn === 'function'
                ? function (...args) {
                    if (window.VibeMe && VibeMe.state && VibeMe.state.beepEnabled === false)
                        return;
...
(function () {
    if (window.__VIBE_QUOTES_BOOTSTRAPPED)
        return;
    ...
    function boot() {
        return __awaiter(this, void 0, void 0, function* () {
            if (document.readyState === 'loading') {
                yield new Promise(r => document.addEventListener('DOMContentLoaded', r, { once: true }));
            }
            const data = yield (window.VIBE_QUOTES_PROMISE || Promise.resolve({ categories: {} }));
            if (window.VibeMe) {
...
(function () {
    ...
    let size = 'expanded';
    try {
        pinned = localStorage.getItem(PIN_KEY) === 'true';
    }
    catch (e) {
        console.warn('Failed to read rail state:', e);
    }
    try {
        size = localStorage.getItem(SIZE_KEY) || 'expanded';
    }
    catch (e) {
        console.warn('Failed to read rail state:', e);
    }
    ...
    if (pinned) {
        ...
        pinBtn === null || pinBtn === void 0 ? void 0 : pinBtn.setAttribute('aria-pressed', 'true');
    }
    collapseBtn === null || collapseBtn === void 0 ? void 0 : collapseBtn.setAttribute('aria-expanded', String(size === 'expanded'));
    const cleanupFns = [];
        ...
        addEvent(pinBtn, 'click', () => {
            ...
            if (pinned) {
                ...
                try {
                    ...
                    console.warn('Failed to save rail state:', e);
                }
    ...
    function handleClickOutside(e) {
        if (pinned)
            return; // Do nothing if the rail is pinned
    ...
    addEvent(document, 'rail:action', ({ detail: { action } }) => {
        ...
        switch (action) {
            case 'new':
                ...
                break;
            case 'fav':
                (_a = document.getElementById('favorites-toggle')) === null || _a === void 0 ? void 0 : _a.click();
                break;
            case 'bookmarks':
            ...
            case 'share':
                ...
                break;
            case 'tts':
                if ((_b = VibeMe.settings) === null || _b === void 0 ? void 0 : _b.tts) {
                    VibeMe.settings.tts.enabled = !VibeMe.settings.tts.enabled;
                    if (!VibeMe.settings.tts.enabled)
                        (_c = VibeMe.tts) === null || _c === void 0 ? void 0 : _c.stop();
                    (_d = VibeMe.bus) === null || _d === void 0 ? void 0 : _d.emit('settings:changed', { key: 'tts.enabled', value: VibeMe.settings.tts.enabled });
                }
                break;
            case 'theme':
                (_e = document.getElementById('settings-toggle')) === null || _e === void 0 ? void 0 : _e.click();
                break;
            case 'settings':
                (_f = document.getElementById('settings-toggle')) === null || _f === void 0 ? void 0 : _f.click();
                break;
            case 'about':
